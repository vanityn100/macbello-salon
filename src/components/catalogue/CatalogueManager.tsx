...
import { 
  ArrowLeft, Edit2, Trash2, Loader2, Sparkles, AlertCircle, Scissors
} from "lucide-react";
...
export default function CatalogueManager({ portalType, pageTitle }: CatalogueManagerProps) {
  ...
  const client = portalType === "admin" ? supabaseAdminClient : supabaseStaffClient;
  ...
  useEffect(() => {
    ...
    const authSub = client.auth.onAuthStateChange((_event, session) => {
      if (session) {
        const role = session.user?.app_metadata?.role;
        if (role === "staff" || role === "admin") {
          setSessionToken(session.access_token);
  ...
  const filteredServices = useMemo(() => {
    return items.filter((i) => {
      const matchesCategory = i.category === "Service" || i.category === "Retail";
    ...
    return (
      <main className="min-h-screen bg-luxury-black flex items-center justify-center px-6">
        <div className="max-w-md w-full border border-gold-primary/20 bg-white/[0.01] p-8 text-center relative shadow-[0_20px_50px_rgba(0,0,0,0.8)]">
          ...
          <p className="text-xs text-ivory/60 font-light mb-6">{authError || "Please log in."}</p>
          <Link
            href={portalType === "admin" ? "/admin" : "/staff"}
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-white/5 pb-6 mb-10 gap-4">
          <div>
            ...
            <p className="text-[10px] text-ivory/40 font-light mt-1 uppercase tracking-wider">
              Managing database for: <span className="text-gold-primary/80">{userEmail}</span>
            </p>
          </div>
        </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="flex flex-col">
                ...
                <input
                  ...
                  placeholder="e.g. Luxury Haircut / Styling Gel"
