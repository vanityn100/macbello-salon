...
if (!page.includes('Fragment')) {
  page = page.replace('import { useState, useEffect } from "react";', 'import { useState, useEffect, Fragment } from "react";');
}
...
// Proper multiline replacement for Fragment
page = page.replace('{filtered.map(p => (\\n                <tr key={p.productId} className="border-b border-white/5 hover:bg-white/[0.01]">', '{filtered.map(p => (\\n                <Fragment key={p.productId}>\\n                <tr className="border-b border-white/5 hover:bg-white/[0.01] cursor-pointer group" onClick={(e) => { if ((e.target as HTMLElement).tagName !== "BUTTON") toggleRow(p.productId); }}>');

// Or safer:
page = page.replace('{filtered.map(p => (', '{filtered.map(p => (\\n                <Fragment key={p.productId}>');
page = page.replace('<tr key={p.productId} className="border-b border-white/5 hover:bg-white/[0.01]">', '<tr className="border-b border-white/5 hover:bg-white/[0.01] cursor-pointer group" onClick={(e) => { if ((e.target as HTMLElement).tagName !== "BUTTON") toggleRow(p.productId); }}>');
...
// Fix the newline injection issue!
