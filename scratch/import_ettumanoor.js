...
async function runImport() {
  console.log("Starting Ettumanoor data import...");
  
  const workbook = xlsx.readFile('ETTUMANOOR SALES REPORT JUNE TILL 27-6-2026.xlsx');
  const sheet = workbook.Sheets[workbook.SheetNames[0]];
  const data = xlsx.utils.sheet_to_json(sheet, { defval: "" });

  const invoiceGroups = {};
  for (const row of data) {
    const invNo = row['Invoice No.'] || row['Invoice No'];
    if (!invNo) continue;
  ...
  for (const invNo of invoices) {
    ...
    const { data: insertedInv, error: invErr } = await supabase.from('invoices').insert({
      invoice_number: `INV-20260628-ETTUMANOOR-${invNo}`,
